...
package com.example.snippet.gravitysensor;
...
public class CustomView extends View {
    ...
    public void setValue(float x, float y, float z){
        ...
        if(x < 0.0f){
            right1 = x2;
            left1 = right1 + len * (x / r);
        }
        else {
            left1 = x2;
            right1 = left1 + len * (x / r);
        }

        // y -  axis
        if(y < 0.0f){
            right2 = x2;
            left2 = right2 + len * (y / r);
        }
        else {
            left2 = x2;
            right2 = left2 + len * (y / r);
        }

        // z - axis
        if(z < 0.0f){
            right3 = x2;
            left3 = right3 + len * (z / r);
        }
        else {
            left3 = x2;
            right3 = left3 + len * (z / r);
        }
